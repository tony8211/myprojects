...
// my global data
#define C_PERIOD_MILLIS      10                  // Number of milliseconds in one processing period
#define C_ACCEL_GAIN         0.0054931640625     // Gain that scales the accelerometer to degrees, based on full scale = 90 degrees
#define C_ACCEL_OFFSET       -450.0              // Offset of accelerometer data, because it is not calibrated to 0
#define C_GYRO_GAIN          0.030517578125      // Gain that scales the gyro to degrees based on +/-250deg/S with full scale = 16384
#define C_GYRO_OFFSET        -84.0               // Offset of gryo data, because it is not calibrated to 0
#define C_KP                 0.5                 // PID coefficient for the proportional part
#define C_KD                 0.2                 // PID coefficient for the derivative part
#define C_KI                 0.07                // PID coefficient for the integral part
#define C_LEFT_MOTOR_SPEED   3                   // The PWM pin assigned to the left motor
#define C_RIGHT_MOTOR_SPEED  10                  // The PWM pin assigned to the right motor
#define C_LEFT_MOTOR_DIR1    2                   // The direction 1 pin assigned to the left motor
#define C_LEFT_MOTOR_DIR2    4                   // The direciton 2 pin assigned to the left motor
#define C_RIGHT_MOTOR_DIR1   9                   // The direction 1 pin assigned to the right motor
#define C_RIGHT_MOTOR_DIR2   11                  // The direciton 2 pin assigned to the right motor
#define C_MOTOR_SCALE        8.5                 // The PWM pins go from 0 - 255, the motorOut calculation +/-30, this scales 30 to 255
float P=0.0;
float I=0.0;
float D=0.0;
...
{      
 ...
 /* Serial.print(F("PWR_MGMT_2 : "));
  ...
  // Set up the PWM and direction pins
  // Left motor
  pinMode(C_LEFT_MOTOR_SPEED, OUTPUT);
  pinMode(C_LEFT_MOTOR_DIR1, OUTPUT);
  pinMode(C_LEFT_MOTOR_DIR2, OUTPUT);
  
  // Right motor
  pinMode(C_RIGHT_MOTOR_SPEED, OUTPUT);
  pinMode(C_RIGHT_MOTOR_DIR1, OUTPUT);
  pinMode(C_RIGHT_MOTOR_DIR2, OUTPUT);
...
//  static uint32_t lastMilliseconds = 0;
  ...
  float motorOut=0.0;
  uint8_t leftMotorOut;
  uint8_t rightMotorOut;


  // read starting milliseconds to time loop
  milliseconds = millis();
  
  // process only once every C_PERIOD_MILLIS
  if((milliseconds % C_PERIOD_MILLIS) == 0)
  {
    Serial.println("");
   ...
   // SWAP (accel_t_gyro.reg.z_gyro_h, accel_t_gyro.reg.z_gyro_l);
    ...
    // convert accel data to angle in degrees, scale for 1g, 
    // for the MPU-6050, set at 2g/S sensitivity, each 16384 LSBs is 1g
    // offset of 450 found through experimentation
    accelX = (((float)accel_t_gyro.value.x_accel+C_ACCEL_OFFSET)*C_ACCEL_GAIN);
    
    // conver raw gyro data to account for horizon, for the MPU-6050,
    // set at +/- 250 deg/S sensitivity, each 131 LSBs is 1 degree/S, there are
    // 1000/C_PERIOD_MILLIS period in 1 second.
    gyroY = ((float)accel_t_gyro.value.y_gyro+C_GYRO_OFFSET)*C_GYRO_GAIN;
        
    // send raw accel and gyro readings to complementary filter
    angle = Complementary(accelX,gyroY,((float)C_PERIOD_MILLIS/1000.0)) ;
    angleError = 0.0-angle;

    // PID algorithm to calculate motorOut, which will be an "angle", the only
    // useable part of the angle is +/- 30 degrees, more than that and the
    // complementary filter loses accuracy. So +/- 30 degrees are the "full-scale"
    // values for motorOut. The PWM pins go from 0 to 255, so we rescale accordingly
    P = angleError * (float)C_KP;
    I = (angleError * ((float)C_PERIOD_MILLIS/1000.0)) * (float)C_KI;
    D = (angleError - lastAngleError) / ((float)C_PERIOD_MILLIS/1000.0) * (float)C_KD;
    motorOut = P + I - D;
    if(motorOut > 30) motorOut = 30.0;
    if(motorOut < -30) motorOut = -30.0;
    
    // control left motor
    controlLeftMotor(motorOut);
    
    // right motor
    controlRightMotor(motorOut);

    lastAngleError = angleError;
    //Serial.print(milliseconds-lastMilliseconds);
...
float Complementary(float x_acc, float gyro, float looptime) {
  static float angle = 0;

  angle = (0.94)*(angle + (gyro*looptime)) + (0.06)*(x_acc);
  
  return angle;
}

// left motor control
void controlLeftMotor(float motorOut)
{
  uint8_t leftMotorOut=0;
  
    // calculate leftMotorOut by scaling motorOut, if motorOut is less than .5 degrees
    // don't turn on motor, eliminating micro-corrections
    // spin forwards
    if(motorOut > 0.5)
    {
      // PWM value
      leftMotorOut = motorOut*C_MOTOR_SCALE;
      analogWrite(C_LEFT_MOTOR_SPEED, leftMotorOut);
      
      // direction pins
      digitalWrite(C_LEFT_MOTOR_DIR1, HIGH);
      digitalWrite(C_LEFT_MOTOR_DIR2, LOW);
    }
    // spin backwards
    else if(motorOut < 0.5)
    {
      // PWM value
      leftMotorOut = (0-motorOut)*C_MOTOR_SCALE;
      analogWrite(C_LEFT_MOTOR_SPEED, leftMotorOut);
      
      // direction pins
      digitalWrite(C_LEFT_MOTOR_DIR1, LOW);
      digitalWrite(C_LEFT_MOTOR_DIR2, HIGH);
    }
    // turn off motor PWM output
    else analogWrite(C_LEFT_MOTOR_SPEED, 0);
}

// right motor control
void controlRightMotor(float motorOut)
{
  uint8_t rightMotorOut=0;

    // calculate rightMotorOut by scaling motorOut, if motorOut is less than .5 degrees
    // don't turn on motor, eliminating micro-corrections
    // spin forwards
    if(motorOut > 0.5)
    {
      // PWM value
      rightMotorOut = motorOut*C_MOTOR_SCALE;
      analogWrite(C_RIGHT_MOTOR_SPEED, rightMotorOut);
      
      // direction pins
      digitalWrite(C_RIGHT_MOTOR_DIR1, LOW);
      digitalWrite(C_RIGHT_MOTOR_DIR2, HIGH);
    }
    // spin backwards
    else if(motorOut < 0.5)
    {
      // PWM value
      rightMotorOut = (0-motorOut)*C_MOTOR_SCALE;
      analogWrite(C_RIGHT_MOTOR_SPEED, rightMotorOut);
      
      // direction pins
      digitalWrite(C_RIGHT_MOTOR_DIR1, HIGH);
      digitalWrite(C_RIGHT_MOTOR_DIR2, LOW);
    }
    // turn off motor PWM output
    else analogWrite(C_RIGHT_MOTOR_SPEED, 0);
}
